...
namespace feishu\open_apis\sheets\v2\spreadsheets\values_batch_get;

use feishu\Contract\RequestInterface;

/**
 * ValuesBatchGetGetRequest 读取多个范围
 * 文档：https://open.feishu.cn/document/server-docs/docs/sheets-v3/data-operation/reading-multiple-ranges
 * API：GET /open-apis/sheets/v2/spreadsheets/:spreadsheetToken/values_batch_get
 */
class ValuesBatchGetGetRequest implements RequestInterface
{
    /** @var string 电子表格 token（路径参数） */
    private $spreadsheetToken;

    /** @var string 多个查询范围，逗号分隔，如 Q7PlXT!A2:B6,0b6377!B1:C8 */
    private $ranges;
    ...
    /**
     * __construct 构造读取多个范围请求
     * @param string $spreadsheetToken 电子表格 token
     * @param string|array $ranges 多个查询范围（字符串逗号分隔，或范围数组）
    ...
    public function __construct(
        ...
        $this->ranges = is_array($ranges)
            ? implode(',', array_map('strval', $ranges))
            : (string)$ranges;
    ...
    {
        return '/sheets/v2/spreadsheets/' . rawurlencode($this->spreadsheetToken) . '/values_batch_get';
    }
